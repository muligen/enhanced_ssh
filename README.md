...
3. 机器 IP 填目标服务器地址；在 AccessClient 连接配置中单独填写堡垒机地址（域名或 IP，不含协议）、堡垒机端口（默认 22）和 AccessClient 账号。每台机器可使用不同的堡垒机。新建或修改堡垒机地址/端口后，工具按该堡垒机地址与账号匹配 PuTTY 共享连接，并保留已记录的远端主机名校验。已有配置未改堡垒机地址/端口时保留原共享标识。
...
同一堡垒机地址、端口和账号对应的共享标识不能分配给多台机器；保存时会拒绝冲突。需要同时连接同一堡垒机下的多台资产时，须由 AccessClient 提供各自独立的共享标识，不能仅靠机器 IP 区分共用入口的会话。
